...
Run `npx krux init` to create the file automatically, or create it manually:
...
The component style to use. Currently only `"krux"` is supported.
...
The registry URL to fetch components from. Defaults to `https://krux.dev/r`.
