---
title: CLI
description: Use the mono-ui CLI to add components to your project.
---

The `mono-ui` CLI lets you initialize your project and add components from the registry.

## init

Initialize mono-ui in a new project:
...
```bash
npx mono-ui add button card dialog select
```
...
```bash
npx mono-ui add button --dry-run
```
...
```
  ...
  Run `mono-ui add --yes card` to update
```
