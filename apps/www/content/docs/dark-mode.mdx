...
mono-ui ships with full dark mode support using CSS variables and `next-themes`.
...
mono-ui uses a `.dark` class on the `<html>` element to switch themes. All CSS variables are redefined under `.dark`:
